...
exports.run = async (client, message, args) => {
    if (!db.get("youtuber" + message.author.id)) return message.reply("İlk Önce Youtuber Olmalısın <a:unlem:909129211351027792>    ");
    ...
    if (daily !== null && timeout - (Date.now() - daily) > 0) {
    ...
    } else {
       {
        db.set(`günlükkullanımgodareçdare-${message.author.id}`, Date.now())
         {
            const randomizer = getRandomInt(100, 3000)
...
exports.conf = {
    enabled: true,
    guildOnly: false,
    aliases: ['ytvideo','ytvç','video-çek'],
    permLevel: 0,
   
}

exports.help = {
    name: 'youtube-video-çek',
    description: '',
    usage: ''
}